...
/// @notice Modified from @uniswap/v3-periphery:
///     (1) _fees() changed from private to internal
///     (2) uses factory.getPool() instead of PoolAddress.computeAddress() to support any V3-compatible factory
library PositionValue {
    ...
    /// @dev CHANGES FROM ORIGINAL:
    /// - visibility changed from private -> internal
    /// - pool lookup uses factory.getPool() instead of CREATE2 computation to remove init code hash dependency
        ...
        (uint256 poolFeeGrowthInside0LastX128, uint256 poolFeeGrowthInside1LastX128) =
            _getFeeGrowthInside(
                IUniswapV3Pool(
                    ...
                    ).getPool(feeParams.token0, feeParams.token1, feeParams.fee)
                ),
                feeParams.tickLower,
                feeParams.tickUpper
            );
