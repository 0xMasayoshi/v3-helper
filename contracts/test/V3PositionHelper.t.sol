...
contract V3PositionHelperTest is Test {
    ...
    function testFetchAllUserPositions() public {
        ...
        Position[] memory positions = positionHelper.getUserPositions(
            positionManager,
            TEST_USER,
            0,
            total
        ...
        for (uint256 i = 0; i < positions.length; i++) {
            assertTrue(
                positions[i].token0 != address(0),
                "token0 should not be zero"
            );
        }
    }
    ...
    function testFetchUserPositionsInPages() public {
        ...
        // Fetch first half
        Position[] memory firstHalf = positionHelper.getUserPositions(
            positionManager,
            TEST_USER,
            0,
            half
        ...
        Position[] memory secondHalf = positionHelper.getUserPositions(
            positionManager,
            TEST_USER,
            half,
            total
        ...
        assertTrue(
            firstHalf[half - 1].tokenId != secondHalf[0].tokenId,
    ...
    function testSkipBeyondBalanceReturnsEmpty() public {
        uint256 total = positionManager.balanceOf(TEST_USER);
        Position[] memory empty = positionHelper.getUserPositions(
            positionManager,
            TEST_USER,
            total + 10,
            5
        ...
        assertEq(
            empty.length,
            0,
            "Expected no positions when skipping past end"
        );
    }
    ...
    function testFetchSpecificPositions() public {
        ...
        for (uint256 i = 0; i < samples.length; i++) {
            ...
            Position memory pos = positionHelper.getPosition(
                positionManager,
                id
            );
            ...
            (
                ...
                uint128 storedOwed0,
                uint128 storedOwed1
            ) = positionManager.positions(id);
            assertTrue(
                pos.tokensOwed0 >= storedOwed0,
                "tokensOwed0 should be >= storedOwed0"
            );
            assertTrue(
                pos.tokensOwed1 >= storedOwed1,
                "tokensOwed1 should be >= storedOwed1"
            );
        }
    }
}
